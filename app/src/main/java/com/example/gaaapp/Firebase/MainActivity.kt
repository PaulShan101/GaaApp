...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        list_view!!.onItemClickListener =
                AdapterView.OnItemClickListener { adapterView, view, position, id ->
                    ...
                    val modify_intent = Intent(applicationContext, ModifyActivity::class.java)
                    modify_intent.putExtra("name", user?.name)
                    modify_intent.putExtra("address", user?.address)
                    modify_intent.putExtra("score", user?.score)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        if (id == R.id.add_record) {
            val add_mem = Intent(this, AddActivity::class.java)
